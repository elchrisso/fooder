  ...
  render () {
    ...
    return (
      <div>
        <CardGroup>
          {recipes.map((recipe) => {
            return <RecipeCard recipeName={recipe.name} recipeDescription={recipe.description} recipeCreator={recipe.User.profile.fullName} recipeId={recipe.id} key={recipe.id}/>
          })}
        </CardGroup>
      </div>
    )
